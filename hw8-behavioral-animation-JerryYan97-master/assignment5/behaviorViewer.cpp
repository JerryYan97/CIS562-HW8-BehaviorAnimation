...
void BehaviorViewer::initializeGui()
{
	...
	m_TwBehaviorType = TwDefineEnum("BehaviorType", beTypeEV, 14);
	...
	TwAddVarRW(m_TwBehaviorBar, "Max number of agents in a row", TW_TYPE_UINT32, &BehaviorController::row_num, "");
}
...
void BehaviorViewer::reset(int numCharacters, int numObstacles)
{
	...
	for (int i = 0; i < m_numObstacles; i++)
	{
		...
		m_Obstacles[i].m_Center.setGlobalTranslation(vec3((x - 0.5)*2000.0, 0.0, (z - 0.5)*2000.0));
	}
...
void BehaviorViewer::onMouseMotion(int x, int y)
{
	if (mModifierState == GLUT_ACTIVE_CTRL) // move target
	{
		...
		mTarget.setGlobalTranslation(intersection);
		//int i = 0;
	}
	else
	{
...
// end new
	...
	// Draw target
	vec3 tpos = mTarget.getGlobalTranslation();
	...
	for (int i = 0; i < m_AgentList.size(); i++)
	{
		...
		else
		{
			...
			vec3 rootPos = pSkeleton->getRootNode()->getGlobalTranslation();
			...
			mat3 Rmat = m_AgentList[i].getBehaviorController()->getGuide().getGlobalRotation();  // body to world rotation matrix
	...
	if (m_ShowObstacles)
	{
		...
		{
			vec3 opos = m_Obstacles[i].m_Center.getGlobalTranslation();
